...
We inforce rate limiting at our API web servers. The Commando.io API rate limit is a maximum of 20 requests per minute based off IP address with burst of 8 requests. Essentially this breaks down to a request every 3 seconds.
...
The `status` property can be one of `QUEUED`, `RUNNING`, `ERROR`, `COMPLETED`. The `events[]` property contains information about the execution throughout its lifecycle.
